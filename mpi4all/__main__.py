...
from mpi4all.generator.julia import JuliaGenerator
...
def parse_args():
    ...
    julia_gen = cli.add_argument_group('Julia Generator Arguments')
    julia_gen.add_argument('--julia', dest='julia', action='store_true',
                          help='Enable Julia Generator')
    julia_gen.add_argument('--julia-out', dest='julia_out', action='store', metavar='name', default=None,
                          help='Julia output directory, default <out>')
...
def main():
    ...
    try:
        ...
        if args.julia:
            logging.info("Generating Julia bindings")
            julia_out = args.julia_out if args.julia_out else args.out
            JuliaGenerator(out=julia_out).build(mpi_info)
            logging.info("Julia bindings Ready")
